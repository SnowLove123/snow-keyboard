 ...
 * @LastEditTime: 2023-06-15 11:07:06
...
export class SnowKeyboard {
  ...
  private backSpaceEvent() {
    ...
    if (this.type == Mode['en'] && this.association.hasAssociationKeyword()) {
      this.showAssociation = false
      this.keyboard.showCandidateWordWapper(false)
    }
  }
  ...
  private enterEvent() {
    ...
    if (this.type == Mode['en'] && this.showAssociation) {
      this.showAssociation = false
      this.keyboard.showCandidateWordWapper(false)
    }
  }
  ...
  private changeModeEvent() {
    this.type = this.type == Mode['zh'] ? Mode['en'] : Mode['zh']
    // 获取布局及根据类型判断当前字典数据
