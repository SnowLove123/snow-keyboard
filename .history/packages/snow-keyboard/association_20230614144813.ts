 ...
 * @LastEditTime: 2023-06-08 18:41:58
 * @FilePath: /projects/test-collect/项目收集/snow-keyboard/packages/snow-keyboard/association.ts
...
export class Association {
  ...
  constructor(globalEvent: GlobalEvent, dictionary: Dictionary | null) {
    ...
    this.candidateListEl = createNestElement({
      ...
      children: new Array(this.limit).fill(null).map((ele) => ({
        tagName: 'div',
        attribute: {
          class: 'word_wapper',
          'data-keycode': '-12',
    ...
    this.globalEvent
      ...
      .addEventListener('clickKeyTarget', (event: Event) => {
        if (event instanceof CustomEvent) {
          ...
          if ([-10, -11, -12, -14, -15].some((n) => n == keyCode)) {
            this.associationClickEvent(target, keyCode)
          } else this.sssociationwordsbar()
        }
      })
    // 退格键
    this.globalEvent
  ...
  // 联想部分点击事件
  private associationClickEvent(target: Event, keyCode: number) {
    switch (keyCode) {
      case -10:
        this.keyword = ''
  ...
  // 根据光标位置插入内容
  private insertText(input: string | undefined = '') {
    const [text, range] = computeInputSelection(
      'insert',
      SnowKeyboard.inputEl!,
      input || this.keyword,
  ...
  // 删除一位联想部分字符
  public backSpaceAssociationText() {
