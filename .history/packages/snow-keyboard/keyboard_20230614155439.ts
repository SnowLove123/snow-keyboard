 ...
 * @LastEditTime: 2023-06-08 13:31:35
 * @FilePath: /projects/test-collect/项目收集/snow-keyboard/packages/snow-keyboard/keyboard.ts
...
export class Keyboard {
  private keyboardDocumentFragment: DocumentFragment | undefined
  ...
  private render(
    ...
    switch (mode) {
      ...
      case 'full-keyboard':
        ...
        computeBaeFont(size)
        // 创建键盘
        const keyboardEle = document.createDocumentFragment(),
          keyboardWapper = createElement('div', {
        ...
        // 添加键盘
        keyboardEle.appendChild(keyboardWapper)
        document.body.appendChild(keyboardEle)
        this.keyboardDocumentFragment = keyboardEle
  ...
  public toggleKeyboardMode({
    ...
    if (this.mode !== mode) {
      this.mode = mode
      this.render(mode, size, inputEl, layout)
    }
  }
  // 获取键盘元素
  ...
  // 切换键盘模式
  public setKeyboardMode(mode: string, layout: Layout) {
    this.mode = mode
    this.keyboardDocumentFragment &&
      document.body.removeChild(this.keyboardDocumentFragment)
    this.render(mode, this.size, SnowKeyboard.inputEl!, layout)
  }
  // 绑定键盘事件
