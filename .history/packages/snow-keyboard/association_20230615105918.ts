 ...
 * @LastEditTime: 2023-06-15 10:59:17
...
import { Dictionary, Mode } from '@snow-keyboard/dictionaries'
...
export class Association {
    ...
    this.globalEvent
      ...
      .addEventListener('clickKeyTarget', (event: Event) => {
        if (event instanceof CustomEvent) {
          const {
            detail: { target, type, pri, keyCode },
          } = event
          // console.log('pri', pri)
          if (type === Mode['en']) return this.insertText(pri)
          // console.log('pri2', pri)
  ...
  private clearText() {
    if (SnowKeyboard.inputEl instanceof HTMLInputElement) {
      SnowKeyboard.inputEl.value = ''
      SnowKeyboard.inputEl。innerText = ''
    }
  }
